...
template <const unsigned int blockDimX__, const unsigned int blockDimY__>
__global__ void hmpp_codelet__runBicg_loop1_(  int32_t i_21, float * HMPPCG_RESTRICT a, float * HMPPCG_RESTRICT r, float * HMPPCG_RESTRICT s)
{
  int32_t j_1;
  j_1 = (blockDimX__ * blockDimY__ * blockIdx.x  +  threadIdx.y * blockDimX__  +  threadIdx.x);
  bool __hmppcg_guard = (!(j_1 <= 4095));
  if(__hmppcg_guard) { goto __hmppcg_label3; };
  s[j_1] = (s[j_1]) + ((r[i_21]) * (a[(i_21 * 4096) + j_1]));
  __hmppcg_label3:;
} 
...
__global__ void hmpp_codelet__runBicg_loop2_(  float * HMPPCG_RESTRICT a, float * HMPPCG_RESTRICT p, float * HMPPCG_RESTRICT q)
{
  ...
  q[i_3] = 0;
  {
    ...
    {
      q[i_3] = (q[i_3]) + ((a[(i_3 * 4096) + j_2]) * (p[j_2]));
    } 
  }
  __hmppcg_label5:;
} 
...

{
  ...
  };
  {
    ...
    for (i_2 = 0, __hmppcg_end = 4095; i_2 <= __hmppcg_end; i_2 += 1)
    {
      if(1LL)
      {
        unsigned int gridDimX__ = 16LL;
      ...
      #endif
        dim3 dim_block(blockDimX__, blockDimY__);
        hmpp_codelet__runBicg_loop1_<blockDimX__, blockDimY__><<<dim_grid, dim_block, 0LL, kernel_stream>>>(i_2, a.getDeviceAddr(), r.getDeviceAddr(), s.getDeviceAddr());
        if ((hmppcg_status_ = CHECK_STATUS(cudaGetLastError()))) return;
      #if CUDA_VERSION >= 3020
        if((hmppcg_status_ = CHECK_STATUS(cudaEventRecord(kernel_event, kernel_stream)))) return;
