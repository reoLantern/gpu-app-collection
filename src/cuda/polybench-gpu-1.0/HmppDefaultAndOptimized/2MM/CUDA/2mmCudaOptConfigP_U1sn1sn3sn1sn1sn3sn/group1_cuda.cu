...
//
// #pragma hmppcg cpiparam __arg0 INOUT c%hmpp_codelet__twoMMloopa: (1, 2) c%hmpp_codelet__twoMMloopb: (2, 0)
// #pragma hmppcg cpiparam __arg1 INOUT a%hmpp_codelet__twoMMloopa: (1, 0)
// #pragma hmppcg cpiparam __arg2 INOUT b%hmpp_codelet__twoMMloopa: (1, 1)
// #pragma hmppcg cpiparam __arg3 INOUT d%hmpp_codelet__twoMMloopb: (2, 1)
// #pragma hmppcg cpiparam __arg4 INOUT e%hmpp_codelet__twoMMloopb: (2, 2)
// 
// #pragma hmppcg cpicall hmpp_codelet__twoMMloopa(__arg1, __arg2, __arg0): 1
// #pragma hmppcg cpicall hmpp_codelet__twoMMloopb(__arg0, __arg3, __arg4): 2
// 
// 
...
// 
// # 40 "twomm.c"
// void hmpp_codelet__twoMMloopa(DATA_TYPE a[2048][2048], DATA_TYPE b[2048][2048], DATA_TYPE c[2048][2048])
// {
//   int  i, j, k;
// 
...
// 
// # 64 "twomm.c"
// void hmpp_codelet__twoMMloopb(DATA_TYPE c[2048][2048], DATA_TYPE d[2048][2048], DATA_TYPE e[2048][2048])
// {
//   int  i, j, k;
// 
// #pragma hmppcg grid blocksize 32 X 8
...
__global__ void hmpp_codelet__twoMMloopa_loop0_(  float * HMPPCG_RESTRICT a, float * HMPPCG_RESTRICT b, float * HMPPCG_RESTRICT c)
{
  int32_t j_2;
  int32_t i_2;
  j_2 = (blockDimX__ * blockIdx.x  +  threadIdx.x);
  i_2 = (blockDimY__ * blockIdx.y  +  threadIdx.y);
  bool __hmppcg_guard = (!((j_2 <= 2047) & (i_2 <= 2047)));
  if(__hmppcg_guard) { goto __hmppcg_label1; };
  {
    int32_t __hmppcg_end, k_2;
    for (k_2 = 0, __hmppcg_end = 683; k_2 <= __hmppcg_end; k_2 += 1)
    {
      if (k_2 <= 681)
      {
        c[(i_2 * 2048) + j_2] = (c[(i_2 * 2048) + j_2]) + ((a[(i_2 * 2048) + k_2]) * (b[(k_2 * 2048) + j_2]));
        c[(i_2 * 2048) + j_2] = (c[(i_2 * 2048) + j_2]) + ((a[(i_2 * 2048) + (k_2 + 682)]) * (b[((k_2 + 682) * 2048) + j_2]));
        c[(i_2 * 2048) + j_2] = (c[(i_2 * 2048) + j_2]) + ((a[(i_2 * 2048) + (k_2 + 1364)]) * (b[((k_2 + 1364) * 2048) + j_2]));
      } 
      else
      {
        c[(i_2 * 2048) + j_2] = (c[(i_2 * 2048) + j_2]) + ((a[(i_2 * 2048) + (k_2 + 1364)]) * (b[((k_2 + 1364) * 2048) + j_2]));
...
void hmpp_codelet__twoMMloopa(  int &hmppcg_status_, void * __h, const cudaDeviceProp &devProp, cudaStream_t kernel_stream, cudaEvent_t kernel_event, Data<float,DefaultPolicy> & a, Data<float,DefaultPolicy> & b, Data<float,DefaultPolicy> & c)

{
  if(1LL)
  {
    ...
    hmpp_codelet__twoMMloopa_loop0_<blockDimX__, blockDimY__><<<dim_grid, dim_block, 0LL, kernel_stream>>>(a.getDeviceAddr(), b.getDeviceAddr(), c.getDeviceAddr());
...
template <const unsigned int blockDimX__, const unsigned int blockDimY__>
__global__ void hmpp_codelet__twoMMloopb_loop0_(  float * HMPPCG_RESTRICT c_11, float * HMPPCG_RESTRICT d, float * HMPPCG_RESTRICT e)
{
  int32_t j_3;
  int32_t i_3;
  ...
  if(__hmppcg_guard) { goto __hmppcg_label3; };
  {
    ...
    for (k_3 = 0, __hmppcg_end = 683; k_3 <= __hmppcg_end; k_3 += 1)
    {
      if (k_3 <= 681)
      {
        e[(i_3 * 2048) + j_3] = (e[(i_3 * 2048) + j_3]) + ((c_11[(i_3 * 2048) + k_3]) * (d[(k_3 * 2048) + j_3]));
        e[(i_3 * 2048) + j_3] = (e[(i_3 * 2048) + j_3]) + ((c_11[(i_3 * 2048) + (k_3 + 682)]) * (d[((k_3 + 682) * 2048) + j_3]));
        e[(i_3 * 2048) + j_3] = (e[(i_3 * 2048) + j_3]) + ((c_11[(i_3 * 2048) + (k_3 + 1364)]) * (d[((k_3 + 1364) * 2048) + j_3]));
      } 
      else
      {
        e[(i_3 * 2048) + j_3] = (e[(i_3 * 2048) + j_3]) + ((c_11[(i_3 * 2048) + (k_3 + 1364)]) * (d[((k_3 + 1364) * 2048) + j_3]));
...
void hmpp_codelet__twoMMloopb(  int &hmppcg_status_, void * __h, const cudaDeviceProp &devProp, cudaStream_t kernel_stream, cudaEvent_t kernel_event, Data<float,DefaultPolicy> & c_1, Data<float,DefaultPolicy> & d, Data<float,DefaultPolicy> & e)

{
  if(1LL)
  {
    ...
    HMPP_CHECK_BLOCK_BOUNDARY(blockDimX__*blockDimY__);
  #if CUDA_VERSION >= 3020
    c_1.makeStreamWait(kernel_stream);
  ...
  #endif
    dim3 dim_block(blockDimX__, blockDimY__);
    hmpp_codelet__twoMMloopb_loop0_<blockDimX__, blockDimY__><<<dim_grid, dim_block, 0LL, kernel_stream>>>(c_1.getDeviceAddr(), d.getDeviceAddr(), e.getDeviceAddr());
    if ((hmppcg_status_ = CHECK_STATUS(cudaGetLastError()))) return;
  #if CUDA_VERSION >= 3020
    if((hmppcg_status_ = CHECK_STATUS(cudaEventRecord(kernel_event, kernel_stream)))) return;
    c_1.waitOnEvent(kernel_event);
...
HMPP_API int hmpp_start(hmpp_handle_t * __h, int __id, int __async)
{
  int status = 0;
  switch(__id) { 
    case 1: // hmpp_codelet__twoMMloopa(__arg1,__arg2,__arg0)
      hmpp_codelet__twoMMloopa(status, __h, __h->devProp, __h->kernel_stream, __h->kernel_event,   (*__h->__arg1), (*__h->__arg2), (*__h->__arg0));
      return status;
    case 2: // hmpp_codelet__twoMMloopb(__arg0,__arg3,__arg4)
      hmpp_codelet__twoMMloopb(status, __h, __h->devProp, __h->kernel_stream, __h->kernel_event,   (*__h->__arg0), (*__h->__arg3), (*__h->__arg4));
      return status;
  }
  return -1;
}
