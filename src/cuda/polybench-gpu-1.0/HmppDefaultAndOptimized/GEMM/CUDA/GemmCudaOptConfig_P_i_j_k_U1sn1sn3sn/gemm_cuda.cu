...
//
// #pragma hmppcg cpiparam __arg0 IN a%hmpp_codelet__runGemm: (1, 0)
// #pragma hmppcg cpiparam __arg1 IN b%hmpp_codelet__runGemm: (1, 1)
// #pragma hmppcg cpiparam __arg2 INOUT c%hmpp_codelet__runGemm: (1, 2)
// 
// #pragma hmppcg cpicall hmpp_codelet__runGemm(__arg0, __arg1, __arg2): 1
// 
// 
// /* begin of extracted source code for directive set "gemm" */
// 
// 
// # 30 "gemm.c"
...
// 
// # 35 "gemm.c"
// void hmpp_codelet__runGemm(DATA_TYPE a[512][512], DATA_TYPE b[512][512], DATA_TYPE c[512][512])
// {
//   int  i, j, k;
...
// 
// 
// /* end of extracted source code for directive set "gemm" */
// 
// 
//
...
__global__ void hmpp_codelet__runGemm_loop0_(  float * HMPPCG_RESTRICT a, float * HMPPCG_RESTRICT b, float * HMPPCG_RESTRICT c)
{
  int32_t j_1;
  int32_t i_1;
  j_1 = (blockDimX__ * blockIdx.x  +  threadIdx.x);
  i_1 = (blockDimY__ * blockIdx.y  +  threadIdx.y);
  bool __hmppcg_guard = (!((j_1 <= 511) & (i_1 <= 511)));
  if(__hmppcg_guard) { goto __hmppcg_label1; };
  {
    int32_t __hmppcg_end, k_1;
    for (k_1 = 0, __hmppcg_end = 171; k_1 <= __hmppcg_end; k_1 += 1)
    {
      if (k_1 <= 169)
      {
        if (k_1 == 0)
        {
          c[(i_1 * 512) + j_1] = (c[(i_1 * 512) + j_1]) * ((float) (2123));
        } 
        c[(i_1 * 512) + j_1] = (c[(i_1 * 512) + j_1]) + ((((float) (32412)) * (a[(i_1 * 512) + k_1])) * (b[(k_1 * 512) + j_1]));
        if ((k_1 + 170) == 0)
        {
          c[(i_1 * 512) + j_1] = (c[(i_1 * 512) + j_1]) * ((float) (2123));
        } 
        c[(i_1 * 512) + j_1] = (c[(i_1 * 512) + j_1]) + ((((float) (32412)) * (a[(i_1 * 512) + (k_1 + 170)])) * (b[((k_1 + 170) * 512) + j_1]));
        if ((k_1 + 340) == 0)
        {
          c[(i_1 * 512) + j_1] = (c[(i_1 * 512) + j_1]) * ((float) (2123));
        } 
        c[(i_1 * 512) + j_1] = (c[(i_1 * 512) + j_1]) + ((((float) (32412)) * (a[(i_1 * 512) + (k_1 + 340)])) * (b[((k_1 + 340) * 512) + j_1]));
      } 
      else
      {
        if ((k_1 + 340) == 0)
...
void hmpp_codelet__runGemm(  int &hmppcg_status_, void * __h, const cudaDeviceProp &devProp, cudaStream_t kernel_stream, cudaEvent_t kernel_event, Data<float,DefaultPolicy> & a, Data<float,DefaultPolicy> & b, Data<float,DefaultPolicy> & c)
...
HMPP_API int hmpp_start(hmpp_handle_t * __h, int __id, int __async)
{
  int status = 0;
  switch(__id) { 
    case 1: // hmpp_codelet__runGemm(__arg0,__arg1,__arg2)
      hmpp_codelet__runGemm(status, __h, __h->devProp, __h->kernel_stream, __h->kernel_event,   (*__h->__arg0), (*__h->__arg1), (*__h->__arg2));
      return status;
  }
  return -1;
}
