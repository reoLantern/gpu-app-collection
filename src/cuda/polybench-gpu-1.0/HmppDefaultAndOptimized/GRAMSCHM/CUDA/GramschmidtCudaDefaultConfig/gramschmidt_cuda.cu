...
__global__ void hmpp_codelet__runGramSchmidt_loop0_(  int32_t k_11, float * HMPPCG_RESTRICT pA, float * HMPPCG_RESTRICT pR)
{
  ...
  priv_priv_pnrm_1 = 0;
  {
    ...
    {
      priv_priv_pnrm_1 = priv_priv_pnrm_1 + ((pA[(i_1 * 2048) + k_11]) * (pA[(i_1 * 2048) + k_11]));
    } 
  }
  pR[(k_11 * 2048) + k_11] = sqrtf(priv_priv_pnrm_1);
  __hmppcg_label1:;
} 

template <const unsigned int blockDimX__, const unsigned int blockDimY__>
__global__ void hmpp_codelet__runGramSchmidt_loop1_(  int32_t k_12, float * HMPPCG_RESTRICT pA, float * HMPPCG_RESTRICT pQ, float * HMPPCG_RESTRICT pR)
{
  int32_t i_2;
  ...
  pQ[(i_2 * 2048) + k_12] = (pA[(i_2 * 2048) + k_12]) / (pR[(k_12 * 2048) + k_12]);
  __hmppcg_label3:;
} 
