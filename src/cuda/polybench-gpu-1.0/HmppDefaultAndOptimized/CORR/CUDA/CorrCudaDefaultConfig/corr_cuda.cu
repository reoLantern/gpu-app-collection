...
__global__ void hmpp_codelet__runCorr_loop3_(  float * HMPPCG_RESTRICT pdata, float * HMPPCG_RESTRICT psymmat)
{
  int32_t j1_1;
  j1_1 = (blockDimX__ * blockDimY__ * blockIdx.x  +  threadIdx.y * blockDimX__  +  threadIdx.x);
  bool __hmppcg_guard = (!(j1_1 <= 2046));
  if(__hmppcg_guard) { goto __hmppcg_label7; };
  psymmat[((j1_1 + 1) * 2049) + (j1_1 + 1)] = (double) 1.000000000000000E+00;
  {
    int32_t __hmppcg_end, j2_1;
    for (j2_1 = 0, __hmppcg_end = 2046 - j1_1; j2_1 <= __hmppcg_end; j2_1 += 1)
    {
      psymmat[((j1_1 + 1) * 2049) + (j2_1 + ((int32_t) (j1_1 + 2)))] = (double) 0.000000000000000E+00;
      {
        int32_t __hmppcg_end, i_4;
        for (i_4 = 0, __hmppcg_end = 2047; i_4 <= __hmppcg_end; i_4 += 1)
        {
          psymmat[((j1_1 + 1) * 2049) + (j2_1 + ((int32_t) (j1_1 + 2)))] = (psymmat[((j1_1 + 1) * 2049) + (j2_1 + ((int32_t) (j1_1 + 2)))]) + ((pdata[((i_4 + 1) * 2049) + (j1_1 + 1)]) * (pdata[((i_4 + 1) * 2049) + (j2_1 + ((int32_t) (j1_1 + 2)))]));
        } 
      }
      psymmat[((j2_1 + ((int32_t) (j1_1 + 2))) * 2049) + (j1_1 + 1)] = psymmat[((j1_1 + 1) * 2049) + (j2_1 + ((int32_t) (j1_1 + 2)))];
    } 
  }
  __hmppcg_label7:;
} 
...
template <const unsigned int blockDimX__, const unsigned int blockDimY__>
__global__ void hmpp_codelet__runCorr_loop4_(  float * HMPPCG_RESTRICT psymmat)
{
  int32_t i_5;
  i_5 = (blockDimX__ * blockDimY__ * blockIdx.x  +  threadIdx.y * blockDimX__  +  threadIdx.x);
  bool __hmppcg_guard = (!(i_5 <= 0LL));
...

{
  ...
  #endif
    dim3 dim_block(blockDimX__, blockDimY__);
    hmpp_codelet__runCorr_loop3_<blockDimX__, blockDimY__><<<dim_grid, dim_block, 0LL, kernel_stream>>>(pdata.getDeviceAddr(), psymmat.getDeviceAddr());
    if ((hmppcg_status_ = CHECK_STATUS(cudaGetLastError()))) return;
  #if CUDA_VERSION >= 3020
    if((hmppcg_status_ = CHECK_STATUS(cudaEventRecord(kernel_event, kernel_stream)))) return;
    pdata.waitOnEvent(kernel_event);
  ...
  #endif
    dim3 dim_block(blockDimX__, blockDimY__);
    hmpp_codelet__runCorr_loop4_<blockDimX__, blockDimY__><<<dim_grid, dim_block, 0LL, kernel_stream>>>(psymmat.getDeviceAddr());
    if ((hmppcg_status_ = CHECK_STATUS(cudaGetLastError()))) return;
  #if CUDA_VERSION >= 3020
    if((hmppcg_status_ = CHECK_STATUS(cudaEventRecord(kernel_event, kernel_stream)))) return;
    psymmat.waitOnEvent(kernel_event);
