...
__global__ void hmpp_codelet__runMvt_loop0_(  float * HMPPCG_RESTRICT a, float * HMPPCG_RESTRICT x1, float * HMPPCG_RESTRICT y1)
{
  int32_t i_3;
  int32_t outer_i_2;
  i_3 = (blockDimX__ * blockIdx.x  +  threadIdx.x);
  outer_i_2 = (blockDimY__ * blockIdx.y  +  threadIdx.y);
  bool __hmppcg_guard = (!((i_3 <= 1) & (outer_i_2 <= 2047)));
  if(__hmppcg_guard) { goto __hmppcg_label1; };
  {
    ...
    {
      {
        int32_t __hmppcg_end, j_3;
        for (j_3 = 0, __hmppcg_end = 1; j_3 <= __hmppcg_end; j_3 += 1)
        {
          x1[i_3 + ((int32_t) (outer_i_2 * 2))] = (x1[i_3 + ((int32_t) (outer_i_2 * 2))]) + ((a[((i_3 + ((int32_t) (outer_i_2 * 2))) * 4096) + (j_3 + ((int32_t) (outer_j_2 * 2)))]) * (y1[j_3 + ((int32_t) (outer_j_2 * 2))]));
        } 
      }
    } 
  }
    ...
    {
      {
        ...
        {
          x2[i_2] = (x2[i_2]) + ((a[((j_4 + ((int32_t) (outer_j_4 * 2))) * 4096) + i_2]) * (y2[j_4 + ((int32_t) (outer_j_4 * 2))]));
        } 
      }
    } 
  }
