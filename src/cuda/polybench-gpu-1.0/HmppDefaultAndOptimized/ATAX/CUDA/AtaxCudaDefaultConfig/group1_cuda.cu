...
{
  int32_t i_3;
  i_3 = (blockDimX__ * blockDimY__ * blockIdx.x  +  threadIdx.y * blockDimX__  +  threadIdx.x);
  bool __hmppcg_guard = (!(i_3 <= 4095));
  ...
  tmp[i_3] = 0;
  {
    ...
    {
      tmp[i_3] = (tmp[i_3]) + ((a[(i_3 * 4096) + j_2]) * (x[j_2]));
    } 
  }
  __hmppcg_label3:;
} 
...
template <const unsigned int blockDimX__, const unsigned int blockDimY__>
__global__ void hmpp_codelet__ataxLoopb_loop0_(  int32_t i_41, float * HMPPCG_RESTRICT a_11, float * HMPPCG_RESTRICT tmp_11, float * HMPPCG_RESTRICT y_11)
{
  int32_t j_3;
  ...
  y_11[j_3] = (y_11[j_3]) + ((a_11[(i_41 * 4096) + j_3]) * (tmp_11[i_41]));
  __hmppcg_label5:;
} 
...
{
  {
    ...
    for (i_4 = 0, __hmppcg_end = 4095; i_4 <= __hmppcg_end; i_4 += 1)
    {
      ...
      #endif
        dim3 dim_block(blockDimX__, blockDimY__);
        hmpp_codelet__ataxLoopb_loop0_<blockDimX__, blockDimY__><<<dim_grid, dim_block, 0LL, kernel_stream>>>(i_4, a_1.getDeviceAddr(), tmp_1.getDeviceAddr(), y_1.getDeviceAddr());
        if ((hmppcg_status_ = CHECK_STATUS(cudaGetLastError()))) return;
      #if CUDA_VERSION >= 3020
        if((hmppcg_status_ = CHECK_STATUS(cudaEventRecord(kernel_event, kernel_stream)))) return;
