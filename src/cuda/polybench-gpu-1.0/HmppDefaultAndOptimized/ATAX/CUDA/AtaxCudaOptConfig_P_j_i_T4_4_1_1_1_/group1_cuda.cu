...
//
// #pragma hmppcg cpiparam __arg0 IN a%hmpp_codelet__ataxLoopa: (1, 0) a%hmpp_codelet__ataxLoopb: (2, 0)
// #pragma hmppcg cpiparam __arg1 INOUT y%hmpp_codelet__ataxLoopa: (1, 2) y%hmpp_codelet__ataxLoopb: (2, 1)
// #pragma hmppcg cpiparam __arg2 INOUT tmp%hmpp_codelet__ataxLoopa: (1, 3) tmp%hmpp_codelet__ataxLoopb: (2, 2)
// #pragma hmppcg cpiparam __arg3 IN x%hmpp_codelet__ataxLoopa: (1, 1)
// 
// #pragma hmppcg cpicall hmpp_codelet__ataxLoopa(__arg0, __arg3, __arg1, __arg2): 1
// #pragma hmppcg cpicall hmpp_codelet__ataxLoopb(__arg0, __arg1, __arg2): 2
// 
// 
...
// 
// # 42 "atax.c"
// void hmpp_codelet__ataxLoopa(DATA_TYPE a[4096][4096], DATA_TYPE x[4096], DATA_TYPE y[4096], DATA_TYPE tmp[4096])
// {
//   int  i, j;
// 
...
// 
// # 70 "atax.c"
// void hmpp_codelet__ataxLoopb(DATA_TYPE a[4096][4096], DATA_TYPE y[4096], DATA_TYPE tmp[4096])
// {
//   int  i, j;
// 
// #pragma hmppcg grid blocksize 32 X 8
...
__global__ void hmpp_codelet__ataxLoopa_loop0_(  float * HMPPCG_RESTRICT y)
{
  int32_t i_5;
  int32_t outer_i_2;
  i_5 = (blockDimX__ * blockIdx.x  +  threadIdx.x);
  outer_i_2 = (blockDimY__ * blockIdx.y  +  threadIdx.y);
  bool __hmppcg_guard = (!((i_5 <= 3) & (outer_i_2 <= 1023)));
  if(__hmppcg_guard) { goto __hmppcg_label1; };
  y[i_5 + ((int32_t) (outer_i_2 * 4))] = 0;
  __hmppcg_label1:;
} 

template <const unsigned int blockDimX__, const unsigned int blockDimY__>
__global__ void hmpp_codelet__ataxLoopa_loop1_(  float * HMPPCG_RESTRICT a, float * HMPPCG_RESTRICT tmp, float * HMPPCG_RESTRICT x)
{
  int32_t i_6;
  int32_t outer_i_4;
  i_6 = (blockDimX__ * blockIdx.x  +  threadIdx.x);
  outer_i_4 = (blockDimY__ * blockIdx.y  +  threadIdx.y);
  bool __hmppcg_guard = (!((i_6 <= 3) & (outer_i_4 <= 1023)));
  if(__hmppcg_guard) { goto __hmppcg_label3; };
  tmp[i_6 + ((int32_t) (outer_i_4 * 4))] = 0;
  {
    int32_t __hmppcg_end, j_2;
    for (j_2 = 0, __hmppcg_end = 4095; j_2 <= __hmppcg_end; j_2 += 1)
    {
      tmp[i_6 + ((int32_t) (outer_i_4 * 4))] = (tmp[i_6 + ((int32_t) (outer_i_4 * 4))]) + ((a[((i_6 + ((int32_t) (outer_i_4 * 4))) * 4096) + j_2]) * (x[j_2]));
    } 
  }
  __hmppcg_label3:;
} 

void hmpp_codelet__ataxLoopa(  int &hmppcg_status_, void * __h, const cudaDeviceProp &devProp, cudaStream_t kernel_stream, cudaEvent_t kernel_event, Data<float,DefaultPolicy> & a, Data<float,DefaultPolicy> & x, Data<float,DefaultPolicy> & y, Data<float,DefaultPolicy> & tmp)

{
  ...
  #if CUDA_VERSION >= 3020
    y.makeStreamWait(kernel_stream);
  #else
    if ((hmppcg_status_ = CHECK_STATUS(cudaDeviceSynchronize()))) return;
  #endif
    dim3 dim_block(blockDimX__, blockDimY__);
    hmpp_codelet__ataxLoopa_loop0_<blockDimX__, blockDimY__><<<dim_grid, dim_block, 0LL, kernel_stream>>>(y.getDeviceAddr());
  ...
  if(1LL)
  {
    ...
    tmp.makeStreamWait(kernel_stream);
    x.makeStreamWait(kernel_stream);
  #else
    if ((hmppcg_status_ = CHECK_STATUS(cudaDeviceSynchronize()))) return;
  #endif
    dim3 dim_block(blockDimX__, blockDimY__);
    hmpp_codelet__ataxLoopa_loop1_<blockDimX__, blockDimY__><<<dim_grid, dim_block, 0LL, kernel_stream>>>(a.getDeviceAddr(), tmp.getDeviceAddr(), x.getDeviceAddr());
    if ((hmppcg_status_ = CHECK_STATUS(cudaGetLastError()))) return;
  #if CUDA_VERSION >= 3020
    if((hmppcg_status_ = CHECK_STATUS(cudaEventRecord(kernel_event, kernel_stream)))) return;
    a.waitOnEvent(kernel_event);
    tmp.waitOnEvent(kernel_event);
    x.waitOnEvent(kernel_event);
  #else
    if ((hmppcg_status_ = CHECK_STATUS(cudaDeviceSynchronize()))) return;
  #endif
...
template <const unsigned int blockDimX__, const unsigned int blockDimY__>
__global__ void hmpp_codelet__ataxLoopb_loop0_(  float * HMPPCG_RESTRICT a_11, float * HMPPCG_RESTRICT tmp_11, float * HMPPCG_RESTRICT y_11)
{
  int32_t j_3;
  j_3 = (blockDimX__ * blockDimY__ * blockIdx.x  +  threadIdx.y * blockDimX__  +  threadIdx.x);
  bool __hmppcg_guard = (!(j_3 <= 4095));
  if(__hmppcg_guard) { goto __hmppcg_label5; };
  {
    int32_t __hmppcg_end, i_4;
    for (i_4 = 0, __hmppcg_end = 4095; i_4 <= __hmppcg_end; i_4 += 1)
    {
      y_11[j_3] = (y_11[j_3]) + ((a_11[(i_4 * 4096) + j_3]) * (tmp_11[i_4]));
    } 
  }
  __hmppcg_label5:;
} 

void hmpp_codelet__ataxLoopb(  int &hmppcg_status_, void * __h, const cudaDeviceProp &devProp, cudaStream_t kernel_stream, cudaEvent_t kernel_event, Data<float,DefaultPolicy> & a_1, Data<float,DefaultPolicy> & y_1, Data<float,DefaultPolicy> & tmp_1)

{
  if(1LL)
  {
    ...
    HMPP_CHECK_BLOCK_BOUNDARY(blockDimX__*blockDimY__);
  #if CUDA_VERSION >= 3020
    a_1.makeStreamWait(kernel_stream);
    tmp_1.makeStreamWait(kernel_stream);
    y_1.makeStreamWait(kernel_stream);
  #else
    if ((hmppcg_status_ = CHECK_STATUS(cudaDeviceSynchronize()))) return;
  #endif
    dim3 dim_block(blockDimX__, blockDimY__);
    hmpp_codelet__ataxLoopb_loop0_<blockDimX__, blockDimY__><<<dim_grid, dim_block, 0LL, kernel_stream>>>(a_1.getDeviceAddr(), tmp_1.getDeviceAddr(), y_1.getDeviceAddr());
    if ((hmppcg_status_ = CHECK_STATUS(cudaGetLastError()))) return;
  #if CUDA_VERSION >= 3020
    if((hmppcg_status_ = CHECK_STATUS(cudaEventRecord(kernel_event, kernel_stream)))) return;
    a_1.waitOnEvent(kernel_event);
    tmp_1.waitOnEvent(kernel_event);
    y_1.waitOnEvent(kernel_event);
  #else
    if ((hmppcg_status_ = CHECK_STATUS(cudaDeviceSynchronize()))) return;
  #endif
...
HMPP_API int hmpp_allocateOnHWA(hmpp_handle_t * __h, int major, int minor, const size_t * size, size_t elsize, int dim)
{
  ...
  {
    case HMPPCG_POP_HASH(1,0): // a@hmpp_codelet__ataxLoopa
    case HMPPCG_POP_HASH(2,0): // a@hmpp_codelet__ataxLoopb
    {
      __h->__arg0 = new Data<float,DefaultPolicy>("__arg0", DEFAULT);
      return __h->__arg0->allocate2(dim, size);
    }
    case HMPPCG_POP_HASH(1,2): // y@hmpp_codelet__ataxLoopa
    case HMPPCG_POP_HASH(2,1): // y@hmpp_codelet__ataxLoopb
    {
      __h->__arg1 = new Data<float,DefaultPolicy>("__arg1", DEFAULT);
      return __h->__arg1->allocate2(dim, size);
    }
    case HMPPCG_POP_HASH(1,3): // tmp@hmpp_codelet__ataxLoopa
    case HMPPCG_POP_HASH(2,2): // tmp@hmpp_codelet__ataxLoopb
    {
      __h->__arg2 = new Data<float,DefaultPolicy>("__arg2", DEFAULT);
      return __h->__arg2->allocate2(dim, size);
    }
    case HMPPCG_POP_HASH(1,1): // x@hmpp_codelet__ataxLoopa
    {
...
HMPP_API int hmpp_start(hmpp_handle_t * __h, int __id, int __async)
{
  int status = 0;
  switch(__id) { 
    case 1: // hmpp_codelet__ataxLoopa(__arg0,__arg3,__arg1,__arg2)
      hmpp_codelet__ataxLoopa(status, __h, __h->devProp, __h->kernel_stream, __h->kernel_event,   (*__h->__arg0), (*__h->__arg3), (*__h->__arg1), (*__h->__arg2));
      return status;
    case 2: // hmpp_codelet__ataxLoopb(__arg0,__arg1,__arg2)
      hmpp_codelet__ataxLoopb(status, __h, __h->devProp, __h->kernel_stream, __h->kernel_event,   (*__h->__arg0), (*__h->__arg1), (*__h->__arg2));
      return status;
  }
  return -1;
}
