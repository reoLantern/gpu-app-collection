...
//
// #pragma hmppcg cpiparam __arg0 IN a%hmpp_codelet__runSyrk: (1, 0)
// #pragma hmppcg cpiparam __arg1 INOUT c%hmpp_codelet__runSyrk: (1, 1)
// 
// #pragma hmppcg cpicall hmpp_codelet__runSyrk(__arg0, __arg1): 1
// 
// 
// /* begin of extracted source code for directive set "syrk" */
// 
// 
// # 29 "syrk.c"
...
// 
// # 34 "syrk.c"
// void hmpp_codelet__runSyrk(DATA_TYPE a[1024][1024], DATA_TYPE c[1024][1024])
// {
//   int  i, j, k;
//   DATA_TYPE  temp;
...
// 
// 
// /* end of extracted source code for directive set "syrk" */
// 
// 
//
...
__global__ void hmpp_codelet__runSyrk_loop0_(  float * HMPPCG_RESTRICT c)
{
  int32_t j_1;
  int32_t i_1;
  j_1 = (blockDimX__ * blockIdx.x  +  threadIdx.x);
  i_1 = (blockDimY__ * blockIdx.y  +  threadIdx.y);
  bool __hmppcg_guard = (!((j_1 <= 1023) & (i_1 <= 1023)));
  if(__hmppcg_guard) { goto __hmppcg_label1; };
  c[(i_1 * 1024) + j_1] = (c[(i_1 * 1024) + j_1]) * ((float) (4546));
  __hmppcg_label1:;
} 

template <const unsigned int blockDimX__, const unsigned int blockDimY__>
__global__ void hmpp_codelet__runSyrk_loop1_(  float * HMPPCG_RESTRICT a, float * HMPPCG_RESTRICT c)
{
  int32_t j_2;
  int32_t i_2;
  j_2 = (blockDimX__ * blockIdx.x  +  threadIdx.x);
  i_2 = (blockDimY__ * blockIdx.y  +  threadIdx.y);
  bool __hmppcg_guard = (!((j_2 <= 1023) & (i_2 <= 1023)));
  if(__hmppcg_guard) { goto __hmppcg_label3; };
  {
    int32_t __hmppcg_end, k_1;
    for (k_1 = 0, __hmppcg_end = 511; k_1 <= __hmppcg_end; k_1 += 1)
    {
      float tmp_1;
      tmp_1 = (float) (12435);
      c[(i_2 * 1024) + j_2] = (c[(i_2 * 1024) + j_2]) + ((tmp_1 * (a[(i_2 * 1024) + k_1])) * (a[(j_2 * 1024) + k_1]));
      c[(i_2 * 1024) + j_2] = (c[(i_2 * 1024) + j_2]) + ((tmp_1 * (a[(i_2 * 1024) + (k_1 + 512)])) * (a[(j_2 * 1024) + (k_1 + 512)]));
    } 
  }
  __hmppcg_label3:;
} 

void hmpp_codelet__runSyrk(  int &hmppcg_status_, void * __h, const cudaDeviceProp &devProp, cudaStream_t kernel_stream, cudaEvent_t kernel_event, Data<float,DefaultPolicy> & a, Data<float,DefaultPolicy> & c)

{
  if(1LL)
  {
    ...
    hmpp_codelet__runSyrk_loop0_<blockDimX__, blockDimY__><<<dim_grid, dim_block, 0LL, kernel_stream>>>(c.getDeviceAddr());
  ...
  #endif
    dim3 dim_block(blockDimX__, blockDimY__);
    hmpp_codelet__runSyrk_loop1_<blockDimX__, blockDimY__><<<dim_grid, dim_block, 0LL, kernel_stream>>>(a.getDeviceAddr(), c.getDeviceAddr());
    if ((hmppcg_status_ = CHECK_STATUS(cudaGetLastError()))) return;
  #if CUDA_VERSION >= 3020
    if((hmppcg_status_ = CHECK_STATUS(cudaEventRecord(kernel_event, kernel_stream)))) return;
...
HMPP_API int hmpp_allocateOnHWA(hmpp_handle_t * __h, int major, int minor, const size_t * size, size_t elsize, int dim)
{
  ...
  {
    case HMPPCG_POP_HASH(1,0): // a@hmpp_codelet__runSyrk
    {
      __h->__arg0 = new Data<float,DefaultPolicy>("__arg0", DEFAULT);
      return __h->__arg0->allocate2(dim, size);
    }
    case HMPPCG_POP_HASH(1,1): // c@hmpp_codelet__runSyrk
    {
...
HMPP_API int hmpp_start(hmpp_handle_t * __h, int __id, int __async)
{
  int status = 0;
  switch(__id) { 
    case 1: // hmpp_codelet__runSyrk(__arg0,__arg1)
      hmpp_codelet__runSyrk(status, __h, __h->devProp, __h->kernel_stream, __h->kernel_event,   (*__h->__arg0), (*__h->__arg1));
      return status;
  }
  return -1;
}
