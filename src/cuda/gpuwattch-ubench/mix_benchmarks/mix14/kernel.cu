...
__global__ void tex_bm_kernel( float* out, unsigned size)
{
	...
	if(tid < size){
		for(unsigned i=0; i<ITERATIONS; ++i){
			out[tid] = ConstArray1[(tid+i)%THREADS_PER_BLOCK]+tex1Dfetch(texmem1,tid);
			out[tid*2] = ConstArray2[(tid+i)%THREADS_PER_BLOCK];
			out[tid*3] =  ConstArray3[(tid+i)%THREADS_PER_BLOCK]*I1[(tid+i)%THREADS_PER_BLOCK];
			out[tid*4] = ConstArray4[(tid+i)%THREADS_PER_BLOCK]/tex1Dfetch(texmem4,tid)*I2[tid%THREADS_PER_BLOCK];
			out[tid*5] = ConstArray5[(tid+i)%THREADS_PER_BLOCK]*ConstArray2[(tid+i)%THREADS_PER_BLOCK];
			out[tid*6] = ConstArray6[(tid+i)%THREADS_PER_BLOCK]/I6[(tid+i)%THREADS_PER_BLOCK];
			out[tid*7] = ConstArray7[(tid+i)%THREADS_PER_BLOCK]+I4[(tid+i)%THREADS_PER_BLOCK];
			out[tid*8] = exp(ConstArray8[(tid+i)%THREADS_PER_BLOCK]);
