...
__global__ void tex_bm_kernel( float *A,float* out, unsigned size)
{
	...
	if(tid < size){
		for(unsigned i=0; i<ITERATIONS; ++i){
			...
			out[tid*4] = ConstArray2[(tid+i)%THREADS_PER_BLOCK]*A[(tid+i+3)%THREADS_PER_BLOCK];
			out[tid*5] = ConstArray3[(tid+i)%THREADS_PER_BLOCK]+A[(tid+i+4)%THREADS_PER_BLOCK];
			out[tid*6] = tex1Dfetch(texmem6,tid);
			out[tid*7] = I3[tid%THREADS_PER_BLOCK]+A[(tid+i+6)%THREADS_PER_BLOCK];
			out[tid*8] = ConstArray4[(tid+i)%THREADS_PER_BLOCK];
			out[tid*9] = log2((ConstArray3[(tid+i)%THREADS_PER_BLOCK]));
		}
	}
